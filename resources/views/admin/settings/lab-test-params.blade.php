...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.lab-tests.params.index') }}" class="btn btn-outline-secondary">
...
let changedRows = new Set();
    ...
    function saveNewRow(button) {
        ...
        fetch('{{ route('admin.settings.lab-tests.params.store') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
    ...
    function deleteRow(id) {
        if (confirm('Вы уверены, что хотите удалить этот параметр?')) {
            fetch(`{{ route('admin.settings.lab-tests.params.destroy', '') }}/${id}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
