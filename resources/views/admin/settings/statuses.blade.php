...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Статусы - {{ $statuses->total() }}</h1>
...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.statuses.index') }}" class="btn btn-outline-secondary">
...
let changedRows = new Set();
    ...
    function saveRow(button) {
        ...
        if (rowId) {
            ...
            if (!data.name || !data.name.trim()) {
                showWarning('Название статуса обязательно для заполнения');
                return;
            }
            if (!data.color || !data.color.trim()) {
                showWarning('Цвет статуса обязателен для выбора');
                return;
            }
    ...
    function saveNewRow(button) {
        ...
        .then(data => {
            if (data.success) {
                showSuccess('Статус успешно создан');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                showWarning(data.message || 'Ошибка при создании статуса');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showWarning('Произошла ошибка при создании статуса: ' + error.message);
        });
    }
        ...
        if (confirm('Вы уверены, что хотите удалить этот статус?')) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showSuccess('Статус успешно удален');
                } else {
                    showWarning(data.message || 'Ошибка при удалении статуса');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showWarning('Произошла ошибка при удалении статуса: ' + error.message);
            });
        }
    }
