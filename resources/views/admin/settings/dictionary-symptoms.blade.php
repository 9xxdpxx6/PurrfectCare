...
<div class="card">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover" id="dictionarySymptomsTable">
                <thead>
                    <tr>
                        <th>Название</th>
                        <th>Описание</th>
                ...
                <tbody>
                    @foreach($dictionarySymptoms as $symptom)
                        <tr data-id="{{ $symptom->id }}" data-original="{{ json_encode($symptom->toArray()) }}">
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $symptom->name }}" 
                                       data-field="name" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $symptom->description }}" 
                                       data-field="description" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteRow({{ $symptom->id }})">
...
<script>
    ...
    function addNewRow() {
        const tbody = document.querySelector('#dictionarySymptomsTable tbody');
    ...
    function saveChanges() {
        ...
        // Collect creates
        document.querySelectorAll('#dictionarySymptomsTable tbody tr').forEach(row => {
            if (!row.dataset.id) {
                const data = {};
                row.querySelectorAll('input[data-field]').forEach(input => {
                    data[input.dataset.field] = input.value;
        ...
        Promise.all([
            ...updates.map(update => 
                fetch(`{{ route('admin.settings.dictionary-symptoms.update', '') }}/${update.id}`, {
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
