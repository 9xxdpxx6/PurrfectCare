...
@section('title', 'Симптомы (словарь)')
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Симптомы (словарь) - {{ $dictionarySymptoms->total() }}</h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        <button type="button" class="btn btn-primary" onclick="addNewRow()">
            <i class="bi bi-plus"></i> Добавить симптом
        </button>
    </div>
</div>
...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.symptoms.index') }}" class="btn btn-outline-secondary">
...
<div class="row g-3">
    @foreach($dictionarySymptoms as $symptom)
        <div class="col-12">
            <div class="card h-100 border-0 border-bottom shadow-sm
        @if($loop->iteration % 2 == 1) bg-body-tertiary @endif" data-id="{{ $symptom->id }}" data-original="{{ json_encode(['name' => $symptom->name, 'description' => $symptom->description]) }}">
                ...
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                        ...
                        <div class="mt-auto w-100">
                            @if($symptom->description)
                                <div class="text-muted">
                                    <span>Описание:</span> {{ $symptom->description }}
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="d-none edit-fields">
                        <div class="row g-2">
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control form-control-sm" value="{{ $symptom->name }}" 
                                       data-field="name" onchange="markAsChanged(this)">
                            </div>
                            <div class="col-12">
                                <label class="form-label small text-muted">Описание</label>
                                <textarea class="form-control" rows="3" data-field="description" onchange="markAsChanged(this)">{{ $symptom->description }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-row flex-lg-column gap-2 ms-lg-4 align-self-start mt-3 mt-lg-0">
                        ...
                        <button type="button" class="btn btn-outline-danger" title="Удалить" onclick="deleteRow({{ $symptom->id }})">
...
@if($dictionarySymptoms->isEmpty())
    <div class="text-center py-5">
        <i class="bi bi-clipboard-pulse display-1 text-muted"></i>
        <h3 class="mt-3 text-muted">Симптомы не найдены</h3>
        <p class="text-muted">Добавьте новый симптом.</p>
        <button type="button" class="btn btn-primary" onclick="addNewRow()">
            <i class="bi bi-plus"></i> Добавить симптом
        </button>
    </div>
...
@if($dictionarySymptoms->hasPages())
    <div class="mt-4">
        {{ $dictionarySymptoms->links() }}
    </div>
@endif
...
let changedRows = new Set();
    ...
    function saveRow(button) {
        ...
        if (rowId) {
            ...
            fetch(`{{ route('admin.settings.symptoms.update', '') }}/${rowId}`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    showSuccess('Симптом успешно обновлен');
                    setTimeout(() => {
            ...
            <div class="card h-100 border-0 border-bottom shadow-sm bg-body-tertiary">
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                    ...
                    <div class="flex-grow-1 d-flex flex-column justify-content-between h-100 align-items-start d-none">
                        <h5 class="card-title">Новый симптом</h5>
                        <div class="mt-auto w-100">
                            <div class="text-muted">
    ...
    function saveNewRow(button) {
        ...
        fetch('{{ route('admin.settings.symptoms.store') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
        ...
        .then(data => {
            if (data.success) {
                showSuccess('Симптом успешно создан');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
    ...
    function deleteRow(id) {
        if (confirm('Вы уверены, что хотите удалить этот симптом?')) {
            fetch(`{{ route('admin.settings.symptoms.destroy', '') }}/${id}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
