...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.diagnoses.index') }}" class="btn btn-outline-secondary">
        ...
        @if($loop->iteration % 2 == 1) bg-body-tertiary @endif" data-id="{{ $diagnosis->id }}" data-original="{{ json_encode(['name' => $diagnosis->name, 'description' => $diagnosis->description]) }}">
                ...
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                    ...
                    <div class="d-none edit-fields">
                        <div class="row g-2">
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control" value="{{ $diagnosis->name }}" 
                    ...
                    <div class="d-flex flex-row flex-lg-column gap-2 ms-lg-4 align-self-start mt-3 mt-lg-0">
                        ...
                        <button type="button" class="btn btn-outline-danger" onclick="deleteRow({{ $diagnosis->id }})">
...
let changedRows = new Set();
    ...
    function saveRow(button) {
        ...
        if (rowId) {
            ...
            if (!data.name || !data.name.trim()) {
                showWarning('Название диагноза обязательно для заполнения');
                return;
            }
            
            fetch(`{{ route('admin.settings.diagnoses.update', '') }}/${rowId}`, {
                method: 'POST',
                headers: {
    ...
    function saveNewRow(button) {
        ...
        fetch('{{ route('admin.settings.diagnoses.store') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
        ...
        .then(data => {
            if (data.success) {
                ...
                showWarning(data.message || 'Ошибка при создании диагноза');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showWarning('Произошла ошибка при создании диагноза: ' + error.message);
        });
    }
    ...
    function deleteRow(id) {
        if (confirm('Вы уверены, что хотите удалить этот диагноз?')) {
            fetch(`{{ route('admin.settings.diagnoses.destroy', '') }}/${id}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showWarning(data.message || 'Ошибка при удалении диагноза');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showWarning('Произошла ошибка при удалении диагноза: ' + error.message);
            });
        }
    }
