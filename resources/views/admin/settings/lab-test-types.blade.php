...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Типы анализов - {{ $labTestTypes->total() }}</h1>
...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.lab-tests.types.index') }}" class="btn btn-outline-secondary">
...
let changedRows = new Set();
    ...
    function saveRow(button) {
        ...
        if (rowId) {
            ...
            if (!data.name || !data.name.trim()) {
                showWarning('Название типа анализа обязательно для заполнения');
                return;
            }
            if (!data.price || data.price <= 0) {
                showWarning('Цена должна быть больше нуля');
                return;
            }
            
            fetch(`{{ route('admin.settings.lab-tests.types.update', '') }}/${rowId}`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    showSuccess('Тип анализа успешно обновлен');
                    setTimeout(() => {
    ...
    function saveNewRow(button) {
        ...
        fetch('{{ route('admin.settings.lab-tests.types.store') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
        ...
        .then(data => {
            if (data.success) {
                showSuccess('Тип анализа успешно создан');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                showWarning(data.message || 'Ошибка при создании типа анализа');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showWarning('Произошла ошибка при создании типа анализа: ' + error.message);
        });
    }
    ...
    function deleteRow(id) {
        if (confirm('Вы уверены, что хотите удалить этот тип анализа?')) {
            fetch(`{{ route('admin.settings.lab-tests.types.destroy', '') }}/${id}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showSuccess('Тип анализа успешно удален');
                } else {
                    showWarning(data.message || 'Ошибка при удалении типа анализа');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showWarning('Произошла ошибка при удалении типа анализа: ' + error.message);
            });
        }
    }
