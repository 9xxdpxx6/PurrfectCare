...
@section('title', 'Единицы измерений')
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Единицы измерений - {{ $units->total() }}</h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        <button type="button" class="btn btn-primary" onclick="addNewRow()">
            <i class="bi bi-plus"></i> Добавить единицу
        </button>
    </div>
</div>
...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.units.index') }}" class="btn btn-outline-secondary">
...
<div class="row g-3">
    @foreach($units as $unit)
        ...
        @if($loop->iteration % 2 == 1) bg-body-tertiary @endif" data-id="{{ $unit->id }}" data-original="{{ json_encode(['name' => $unit->name, 'symbol' => $unit->symbol]) }}">
                ...
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                        ...
                        <div class="mt-auto w-100">
                            @if($unit->symbol)
                                <div class="text-muted">
                                    <span>Обозначение:</span> {{ $unit->symbol }}
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-row flex-lg-column gap-2 ms-lg-4 align-self-start mt-3 mt-lg-0">
                        ...
                        <button type="button" class="btn btn-outline-danger" title="Удалить" onclick='deleteRow({{ $unit->id }})'>
...
@if($units->isEmpty())
    <div class="text-center py-5">
        ...
        <h3 class="mt-3 text-muted">Единицы измерений не найдены</h3>
        <p class="text-muted">Добавьте новую единицу измерения.</p>
        <button type="button" class="btn btn-primary" onclick="addNewRow()">
            <i class="bi bi-plus"></i> Добавить единицу
        </button>
    </div>
...
let changedRows = new Set();
    ...
    function saveRow(button) {
        ...
        if (rowId) {
            ...
            if (!data.name || !data.name.trim()) {
                showWarning('Название единицы измерения обязательно для заполнения');
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    showSuccess('Единица измерения успешно обновлена');
                    setTimeout(() => {
            ...
            <div class="card h-100 border-0 border-bottom shadow-sm bg-body-tertiary">
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                    ...
                    <div class="flex-grow-1 d-flex flex-column justify-content-between h-100 align-items-start d-none">
                        <h5 class="card-title">Новая единица</h5>
                        <div class="mt-auto w-100">
                            <div class="text-muted">
                                <span>Обозначение:</span> <span class="short-name-display">Не указано</span>
                            </div>
                        </div>
                    </div>
    ...
    function saveNewRow(button) {
        ...
        .then(data => {
            if (data.success) {
                showSuccess('Единица измерения успешно создана');
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
                showWarning(data.message || 'Ошибка при создании единицы измерения');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showWarning('Произошла ошибка при создании единицы измерения: ' + error.message);
        });
    }
        ...
        if (confirm('Вы уверены, что хотите удалить эту единицу измерения?')) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showSuccess('Единица измерения успешно удалена');
                } else {
                    showWarning(data.message || 'Ошибка при удалении единицы измерения');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showWarning('Произошла ошибка при удалении единицы измерения: ' + error.message);
            });
        }
    }
