            ...
            <table class="table table-hover" id="unitsTable">
                ...
                <tbody>
                    @foreach($units as $unit)
                        <tr data-id="{{ $unit->id }}" data-original="{{ json_encode($unit->toArray()) }}">
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $unit->name }}" 
                                       data-field="name" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $unit->short_name }}" 
                                       data-field="short_name" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteRow({{ $unit->id }})">
...
<script>
    ...
    function saveChanges() {
        ...
        Promise.all([
            ...updates.map(update => 
                fetch(`{{ route('admin.settings.units.update', '') }}/${update.id}`, {
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
