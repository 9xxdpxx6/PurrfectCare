            ...
            <table class="table table-hover" id="suppliersTable">
                ...
                <tbody>
                    @foreach($suppliers as $supplier)
                        <tr data-id="{{ $supplier->id }}" data-original="{{ json_encode($supplier->toArray()) }}">
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $supplier->name }}" 
                                       data-field="name" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $supplier->contact_person }}" 
                                       data-field="contact_person" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $supplier->phone }}" 
                                       data-field="phone" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <input type="email" class="form-control form-control-sm" value="{{ $supplier->email }}" 
                                       data-field="email" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <input type="text" class="form-control form-control-sm" value="{{ $supplier->address }}" 
                                       data-field="address" onchange="markAsChanged(this)">
                            </td>
                            <td>
                                <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteRow({{ $supplier->id }})">
...
<script>
    ...
    function saveChanges() {
        ...
        Promise.all([
            ...updates.map(update => 
                fetch(`{{ route('admin.settings.suppliers.update', '') }}/${update.id}`, {
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
