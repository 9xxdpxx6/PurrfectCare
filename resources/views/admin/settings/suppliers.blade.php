...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Поставщики - {{ $suppliers->total() }}</h1>
...
<form method="GET" class="mb-4">
    <div class="d-flex flex-wrap align-items-end gap-2">
        ...
        <div class="d-flex gap-2 ms-auto w-auto">
                            <a href="{{ route('admin.settings.suppliers.index') }}" class="btn btn-outline-secondary">
...
<div class="row g-3">
    @foreach($suppliers as $supplier)
        ...
        @if($loop->iteration % 2 == 1) bg-body-tertiary @endif" data-id="{{ $supplier->id }}" data-original="{{ json_encode(['name' => $supplier->name]) }}">
                ...
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                    ...
                    <div class="d-flex flex-row flex-lg-column gap-2 ms-lg-4 align-self-start mt-3 mt-lg-0 text-nowrap">
                        ...
                        <button type="button" class="btn btn-outline-danger" title="Удалить" onclick='deleteRow({{ $supplier->id }})'>
...
@if($suppliers->isEmpty())
    <div class="text-center py-5">
        ...
        <p class="text-muted">Добавьте нового поставщика.</p>
        <button type="button" class="btn btn-primary" onclick="addNewRow()">
            <i class="bi bi-plus"></i> Добавить поставщика
        </button>
    </div>
...
let changedRows = new Set();
    ...
    function saveRow(button) {
        ...
        if (rowId) {
            ...
            if (!data.name || !data.name.trim()) {
                showWarning('Название поставщика обязательно для заполнения');
                return;
            }
            ...
            <div class="card h-100 border-0 border-bottom shadow-sm bg-body-tertiary">
                <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                    ...
                    <div class="flex-grow-1 d-flex flex-column justify-content-between h-100 align-items-start d-none">
                        <h5 class="card-title">Новый поставщик</h5>
    ...
    function saveNewRow(button) {
        ...
        fetch('{{ route('admin.settings.suppliers.store') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
        ...
        .then(data => {
            if (data.success) {
                ...
                showWarning(data.message || 'Ошибка при создании поставщика');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showWarning('Произошла ошибка при создании поставщика: ' + error.message);
        });
    }
    ...
    function deleteRow(id) {
        if (confirm('Вы уверены, что хотите удалить этого поставщика?')) {
            fetch(`{{ route('admin.settings.suppliers.destroy', '') }}/${id}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showWarning(data.message || 'Ошибка при удалении поставщика');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showWarning('Произошла ошибка при удалении поставщика: ' + error.message);
            });
        }
    }
